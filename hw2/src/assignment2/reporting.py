...
def write_markdown(root, content):
    path = root / "report" / "hw2_report.md"
...
def _scale_points(values, left_pad, top_pad, plot_width, plot_height):
    ...
    for idx, value in enumerate(values):
        ...
        y = top_pad + plot_height - (value - minimum) * plot_height / (maximum - minimum)
        points.append((x, y))
    return points
...
def multi_line_svg(path, title, series_map, y_label, x_ticks=None):
    ...
    points_map = {name: _scale_points(values, left_pad, top_pad, plot_width, plot_height) for name, values in series_map.items()}
    ...
    if x_ticks is None:
        ...
        for ratio in (0.0, 0.5, 1.0):
            idx = int((sample_len - 1) * ratio)
            tick_points.append((idx, str(idx + 1)))
    else:
        tick_points = x_ticks
    x_tick_positions = []
    ...
    y_ticks = [
        ...
        (height - bottom_pad - (middle - minimum) * plot_height / max(maximum - minimum, 1.0), _format_tick(middle)),
        (top_pad, _format_tick(maximum)),
    ]
    parts = [
        f'<line class="axis-x" x1="{left_pad}" y1="{height - bottom_pad}" x2="{left_pad + plot_width}" y2="{height - bottom_pad}" stroke="black" stroke-width="1"/>',
        f'<line class="axis-y" x1="{left_pad}" y1="{top_pad}" x2="{left_pad}" y2="{height - bottom_pad}" stroke="black" stroke-width="1"/>',
        f'<text x="{left_pad + plot_width / 2:.1f}" y="{height - 12}" font-size="12" text-anchor="middle">时间</text>',
        f'<text class="y-label" x="32" y="{top_pad + plot_height / 2:.1f}" font-size="12" text-anchor="middle" transform="rotate(-90 32 {top_pad + plot_height / 2:.1f})">{y_label}</text>',
        f'<rect class="legend-box" x="{width - right_pad + 20}" y="{top_pad}" width="{right_pad - 35}" height="{max(70, 24 * len(series_map))}" fill="white" stroke="#cccccc" stroke-width="1"/>',
    ]
...
__all__ = ["ensure_dirs", "markdown_table", "multi_line_svg", "write_csv", "write_markdown"]
